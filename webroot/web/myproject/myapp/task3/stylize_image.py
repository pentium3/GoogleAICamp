...
def task3main(options_content, options_mask, options_model_path, options_output_path):
    #parser = build_parser()
    #options = parser.parse_args()
    #check_opts(options)
    
    shape = imread(options_content).shape
    content_image = utils.load_image(options_content,[256,256])
    content_image = np.expand_dims(content_image,axis=0)
    
    mask_image = utils.load_image(options_mask,[64,64])[...,0:1] /255.
    mask_image = np.expand_dims(mask_image,0)
    # mask_image = np.ones((1,64,64,1))
    print(type(mask_image))
    print(mask_image.shape)
    # print(mask_image[0][0][0])
    # print(mask_image[0][3][3])

    model = options_model_path
    prediction = ffwd(content_image,mask_image,model)
    utils.save_image(prediction,[shape[0],shape[1]], options_output_path)
...
#    task3main('cartoon.jpg', 'cartoonmask.jpg', '../modelsfile/rain/conditional_style_network.ckpt', 'op3.jpg')
