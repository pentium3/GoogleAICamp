...
import utils
from conditional_style_transfer import ConditionalStyleTransfer
...
def main():
    ...
    content = utils.load_image(content_targets[0], [256,256])
    content_shape = content.shape
    device = '/gpu:1' if options.use_gpu else '/cpu:0'

    conditional_style_transfer = ConditionalStyleTransfer(
        vgg_path=VGG_PATH,
    ...
    for iteration, network, first_image, losses in conditional_style_transfer.train(
        ...
        if (iteration % 100 == 0):
            saver.save(network, options.save_path + '/conditional_style_network.ckpt')
