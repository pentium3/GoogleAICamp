...
class ConditionalStyleTransfer:
    ...
    def __init__(self, vgg_path,
            ...
            self.net_mask_batch = tf.placeholder(tf.float32, shape = [BATCH_SIZE,64,64,1])
            self.index = tf.placeholder(tf.int32, shape=[BATCH_SIZE,1])
    ...
    def get_mask(self,batch_size):
        ...
        x0= int(random.sample(range(110),1)[0]/4)
        x1 = int(random.sample(range(130,256),1)[0]/4)
        y0= int(random.sample(range(110),1)[0]/4)
        y1= int(random.sample(range(130,256),1)[0]/4)
        ...
        size = np.array([size])
        print("MMMMMMMMMASK",mask_array.shape ,mask_array2.shape ,index.shape ,size)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(epochs):
                for i in range(0, len(content_training_images)-3, self.batch_size):
                    ...
                    mask,mask2,index,size =self.get_mask(BATCH_SIZE)
                    ...
                    if i %100 ==0 :
                        ...
                        utils.save_image(self.output[0], 100,'result.jpg')
